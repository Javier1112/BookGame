...
const TOTAL_ROUNDS = Math.max(1, Number(process.env.GAME_TOTAL_ROUNDS ?? 4));
...
const SCENE_DESCRIPTION_MAX_CHARS = Math.max(
  80,
  Number(process.env.ZHIPU_SCENE_MAX_CHARS ?? 420)
);
const OPTION_TEXT_MAX_CHARS = Math.max(
  6,
...
const RETRY_BACKOFF_MS = (process.env.ZHIPU_RETRY_BACKOFF_MS ?? "1000,2000,4000")
  .split(",")
  .map((value) => Number(value.trim()))
  .filter((value) => Number.isFinite(value) && value > 0);
const MAX_429_RETRIES = Math.max(
  0,
  Number(process.env.ZHIPU_MAX_429_RETRIES ?? 1)
);
const MAX_429_WAIT_MS = Math.max(
  1000,
...
const extractOptions = (
  ...
  const extracted = rawOptions
    .map((item, index) => {
      ...
      const textRaw = typeof record.text === "string" ? record.text.trim() : "";
      const text = clampText(textRaw, OPTION_TEXT_MAX_CHARS);
      if (!text) return null;
...
const validateStoryResponse = (data: unknown): ZhipuStoryResponse => {
  ...
  const scene = clampText(sceneRaw.trim(), SCENE_DESCRIPTION_MAX_CHARS);
  ...
  if (!character || !scene || !imagePrompt) {
    throw new Error("故事生成缺少必需字段，请稍后尝试。");
  }
...
export const createZhipuGameService = (
  ...
  const fetchWithRetry = async (
    ...
    for (let attempt = 0; ; attempt += 1) {
      ...
      if (providerCooldownUntil > now) {
        await sleep(providerCooldownUntil - now);
      }
      ...
      const baseBackoffMs = RETRY_BACKOFF_MS[attempt] ?? RETRY_BACKOFF_MS.at(-1) ?? 1000;
      const rawDelayMs = Math.min(
        MAX_429_WAIT_MS,
        Math.max(baseBackoffMs, retryAfterMs ?? 0)
      );
      const retryInMs = addJitter(rawDelayMs);
  ...
  const buildUserPrompt = (payload: GameTurnRequest) => {
    ...
    const protagonistName = payload.protagonistName?.trim();
    ...
    return `我们正在进行一场基于《${payload.bookTitle}》的互动故事游戏。现在是第 ${
      ...
      protagonistName ? `本局主角固定为“${protagonistName}”，必须保持一致与第一视角。` : ""
    }历史：${stringifyHistory(payload.history)}。${
      isFinalTurn
  ...
  const fetchStory = async (
    ...
    const protagonistRule = protagonistName
      ? `4.1）当前主角固定为“${protagonistName}”，character_name 必须严格等于该名字，不得替换。`
      : "";
...
9）不得包含任何其它键。`;
    ...
    const ensureProtagonist = async (
      story: ZhipuStoryResponse
    ): Promise<ZhipuStoryResponse> => {
      if (!protagonistName || story.character_name === protagonistName) {
        return story;
      }

      logJson("warn", "zhipu_story_protagonist_mismatch", {
        requestId,
        expected: protagonistName,
        actual: story.character_name
      });
...
character_name 必须严格为：${protagonistName}。
如果需要解释身份变化，请在 scene_description 中简要说明原因，但不要改变剧情走向。`;
      const repairUser = `请将下面内容调整为符合要求的 JSON 结构：\n\n内容：\n${JSON.stringify(
        story
      )}`;
      const repaired = await callChatJsonObject(
        repairSystem,
        repairUser,
        1,
        requestId
      );
      const parsed = tryParse(repaired.content);
      const fixed = validateStoryResponse(parsed);
      if (protagonistName && fixed.character_name !== protagonistName) {
        return { ...fixed, character_name: protagonistName };
      }
      return fixed;
    };

    try {
      const story = validateStoryResponse(parsed1);
      return await ensureProtagonist(story);
    } catch {
...
options 必须为 3 个互斥且可执行的具体动作，使用动词开头，每条不超过 ${OPTION_TEXT_MAX_CHARS} 字；禁止“继续/随便/不知道/以上都行/随机/跳过”等空泛选项；除非 is_game_over=true，此时 options 可以为 []。
如果输入缺少 options 或 options 不合法，请根据场景内容补全并生成 3 个合理选项。
image_prompt 需要是中文，包含视觉描述；`;
      ...
      const story = validateStoryResponse(parsed2);
      return await ensureProtagonist(story);
    }
  };
